...
func (p *MongoHandler) setStandardClassBPositionReport(message models.Message) {

	packet, ok := message.Packet.(ais.StandardClassBPositionReport)
	if !ok {
		slog.Error("error processing StandardClassBPositionReport - could not unqueue packet")
		return
	}
