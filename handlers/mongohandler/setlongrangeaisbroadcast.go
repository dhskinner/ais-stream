...
func (p *MongoHandler) setLongRangeAisBroadcastMessage(message models.Message) {

	packet, ok := message.Packet.(ais.LongRangeAisBroadcastMessage)
	if !ok {
		slog.Error("error processing LongRangeAisBroadcastMessage - could not unqueue packet")
		return
	}
